  ...
  try {
    ...
    if (!files.length) {
      res.json({
        ...
        message:
          "I opened the folder, but could not read the file list. TeraBox layout/API may be different for your link."
      });
      return;
    }
...
app.post("/resolve", async (req, res) => {
  ...
  try {
    ...
    // IMPORTANT:
    // Many TeraBox flows do not give a permanent direct URL. Sometimes we can only stream
    // by using a download endpoint. Here we do "best effort": if the network JSON contained
    // something that looks like a direct URL, we use it.
    ...
    if (!directUrl) {
      res.json({
        ...
        error: "NO_DIRECT_URL",
        message:
          "I found the file, but did not get a direct download URL from TeraBox yet. I can adapt the resolver if you share the exact share-link format (without cookies)."
      });
      return;
    }
